...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    secondary:
      "inline-block text-sm rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide text-stone-800 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 text-stone-400 focus:bg-stone-300 focus:text-stone-800 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-yellow-100 px-4 py-2.5 md:px-6 md:py-3.5",
  };
  ...
  // Deal with the button which has event handler
  if (onClick)
    return (
